...
class ExperimentBuilder(object):
    def __init__(self, parser, data):
        ...
        self.gpus = [int(x) for x in args.gpus]
        ...
        dagan = DAGAN(batch_size=self.batch_size, input_x_i=self.input_x_i, input_x_j=self.input_x_j,
                      ...
                      gen_inner_conv=gen_inner_layers, num_gpus=self.num_gpus, gpus = self.gpus, z_dim=self.z_dim, z_inputs=self.z_input)

        self.summary, self.losses, self.graph_ops = dagan.init_train(learning_rate = self.learning_rate)
